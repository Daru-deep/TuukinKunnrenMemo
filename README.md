...
1. 起きたら **行き** タブの「起床時刻」の **今** を押す
2. 出発前に「記録開始」を押す（初回は位置情報と通知の許可を求められる）
3. 出発したら「出発時刻」、自宅の最寄り駅に着いたら「自宅最寄り駅 到着」、
   電車に乗ったら「乗車電車」の **今**
4. 混雑度・遅延を選ぶ
5. 職場の最寄り駅に着いたら「職場最寄り駅 到着」、ビルに着いたら「ビル到着時刻」の **今**
   → 所要時間と判定がその場で出る
6. 「保存する」を押す（GPSの記録も一緒に止まって保存される）

行きの時刻は 起床 → 出発 → 自宅最寄り駅着 → 乗車電車 → 職場最寄り駅着 → ビル到着 の6つ。
**所要時間と判定（オンタイム／注意／遅刻）は「出発」と「ビル到着」で出す**。
起床と駅着は任意で、空のままでも保存できる（後から足した項目なので、それ以前の記録は空欄）。

帰りは出発・乗車電車・到着の3つだけで、判定もしない。**集計・出力** タブで平均・最短ルート・記録一覧を見て、
xlsxを書き出せる。
...
端末なしで走る32件のテストが入っている。

- `CommuteTest` … 日またぎの所要時間、判定の境界（8:29 / 8:30 / 8:59 / 9:00）、集計、GPSの間引き、起床・駅着が行きだけに残ること
- `XlsxWriterTest` … 自作xlsxがZIPとして開けるか、色分け、エスケープ、座標を出力していないこと
- `RecordStoreTest` … 保存・更新・削除、軌跡の分離、壊れたファイルからの復帰、起床・駅着の無い古い記録が読めること
